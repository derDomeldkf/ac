    ...
    function update(month) {
        $.post("backend/getbymonth.php", {"action":"getmonth", "month":month}, function(data2) {
            ...
            for (index=0; index<entries.length; index++) {
                ...
                total+=pay;
                $('.audiolist tbody').append("<tr><td><p>"+entries[index]["name"]+"</p></td><td>"+entries[index]["duration"]+"</td><td>"+entries[index]["wduration"]+"</td><td><img src=\"icons/coin_gold.png\"/>/<img src=\"icons/alarmclock.png\"/> "+(Math.round(payh * 100) / 100).toFixed(2)
